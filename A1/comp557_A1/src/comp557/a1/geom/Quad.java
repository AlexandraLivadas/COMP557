...
public class Quad {
	...
	private static int colorVectorBufferID;
	...
	public static void draw( GLAutoDrawable drawable, BasicPipeline pipeline ) {
		...
		if ( ! initialized ) {
	        ...
	        FloatBuffer colorVectorBuffer = GLBuffers.newDirectFloatBuffer( new float[] { 1, 0.96f, 1, 0.2f} );
	        ...
	        colorVectorBufferID = bufferIDs[2];
	        ...
	        //Color buffer
	        gl.glBindBuffer( GL4.GL_ARRAY_BUFFER, colorVectorBufferID );
	        gl.glBufferData( GL4.GL_ARRAY_BUFFER, colorVectorBuffer.capacity() * Float.BYTES, colorVectorBuffer, GL4.GL_STATIC_DRAW );
		    ...
		    //Color attribute
			gl.glBindBuffer( GL4.GL_ARRAY_BUFFER, colorVectorBufferID );		
		    gl.glVertexAttribPointer( pipeline.colorAttributeID, 3, GL4.GL_FLOAT, false, 3*Float.BYTES, 0 );
			gl.glBindBuffer( GL4.GL_ELEMENT_ARRAY_BUFFER, elementBufferID );
