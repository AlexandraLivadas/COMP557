...
/**
 * PointLightCamera class for light point of view
 * 
...
public class PointLightCamera extends Camera {	
    ...
    public void draw( GLAutoDrawable drawable, ShadowPipeline pipeline ) {
		...
		pipeline.push();
		pipeline.multMatrix(drawable, Vinv); //Specifying view from light
		...
		pipeline.push();
		pipeline.multMatrix(drawable, Vinv); //Get into light coordinate frame
		pipeline.multMatrix(drawable, Pinv); // Create perspective for frustum
		...
		pipeline.push();
		pipeline.multMatrix(drawable, Vinv); //Get into light coord frame
		pipeline.multMatrix(drawable, Pinv); //Create perspective (in light frame coords)
		pipeline.translate(drawable, 0, 0, -1); //Move "back" (forward) by 1 unit 
