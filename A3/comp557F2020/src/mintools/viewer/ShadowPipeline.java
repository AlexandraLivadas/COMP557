...
public class ShadowPipeline {
    ...
    private int PLMatrixID;
    private int VLMatrixID;
    private int sigmaID;
	...
	public ShadowPipeline( GLAutoDrawable drawable ) {
		...
		basicLighting = new GLSLProgram( drawable, "perFragmentLighting" );		
        ...
        // extra uniforms for the shadow map...
        PLMatrixID = gl.glGetUniformLocation(  basicLighting.glslProgramID, "PL" );
        VLMatrixID = gl.glGetUniformLocation(  basicLighting.glslProgramID, "VL" );
        ...
        // Texture and program for drawing depth view
	...
	public void startCameraViewPass( GLAutoDrawable drawable ) {
        ...
        gl.glUniform1f( shininessID, (float) 127.0 ); // default to very shiny (127 is max?)
        gl.glUniform3f( lightColorID, 1, 1, 1 ); // default to white light
        gl.glUniform1i( enableLightingID, 1 );
        
        // extra uniforms if we are using the shadow map...
        currentGLSLProgram.glUniformMatrix( gl, PLMatrixID, light.P );
        currentGLSLProgram.glUniformMatrix( gl, VLMatrixID, light.V );
        gl.glUniform1f( sigmaID, light.sigma.getFloatValue() );
		gl.glUniform1i( shadowMapID, 1 ); // texture unit zero
	}
	...
	public void debugLightTexture( GLAutoDrawable drawable ) {
		...
		gl.glUniform1i( depthTextureID, 1 ); // texture unit zero
	...
	public void loadIdentity( GLAutoDrawable drawable ) {
		stack[stackPos].setIdentity();
		stackInvT[stackPos].setIdentity();
		setModelingMatrixUniform( drawable.getGL().getGL4() );
	}
	
	public void drawTextLines( GLAutoDrawable drawable, String text ) {
		fontTexture.drawTextLines(drawable, text, 40, 40, 32, 1,1,1 );
	}

	public void drawLabel( GLAutoDrawable drawable, String text ) {
		// Where is the origin? projected onto the screen?
		Vector4f vec = new Vector4f(0,0,0,1);
		stack[stackPos].transform(vec);
		camera.V.transform(vec);
		camera.P.transform(vec);
		vec.scale( 1/vec.w );
		...
		startCameraViewPass(drawable); // go back to our basic pipeline... but note regular context switches are expensive :(
	}
